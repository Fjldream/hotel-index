    ...
    <div class="">
        ...
        <header>
            ...
            <div class="header-con">
                <div class="back">
                    <div>
                        <van-image :src="icon.back" @click="pageback"/>
                    </div>
                </div>
                <div class="comment">
        ...
        <main>
            <div class="container clearfix">
                <section class="clearfix topic">
                    ...
                    <div class="rank">
                        <div class="num">{{homestay.hscore}}</div>
                    ...
                    <div>
                        <div class="in float-left">
                            ...
                            <div class="date">{{start}}
                                <span class="week highlight">{{startDay}}</span>
                            </div>
                        </div>
                        <div class="time float-left">
                            <div class="count">共{{stayTime}}晚</div>
                            <div class="underline">
                ...
                <section class="tabs tabs-con-con">
                    <van-tabs>
                        <van-tab title="详情">
                            ...
                            <div class="more">
                                <div class="theme" @click="detailIsMore" v-text="detailismore?'查看更多':'收起'"></div>
                            </div>
...
<!--                                                <div class="recommend-star float-left" v-for="(star,index) in item.star"-->
<!--                                                     :key="index">-->
<!--                                                    <van-image :src="icon.star"></van-image>-->
<!--                                                </div>-->
<!--                                                <div class="recommend-star float-left"-->
<!--                                                     v-for="(star,index) in (5 - item.star)" :key="index">-->
        ...
        methods: {
            initHomestay(hid) {
                apiDetail(hid).then(res => {
                    if (res.code === SUCCESS) {
                        this.homestay = Object.assign({}, res.data.homestay);
                        let bannerArr = this.homestay.hbanner.split(',');
                        this.images = bannerArr.map(ele => IMGURL + ele);
                        this.recommend = res.data.recommend;
                        // for (let i=0;i<this.recommend.length;i++){
                        //
                        // }
                        this.recommend.forEach(function (value) {
                            value.hthumb = IMGURL+value.hthumb;
                        })
                        this.start = this.$store.state.indexSearch.startTime;
                        this.end = this.$store.state.indexSearch.endTime;
                        this.startDay = this.$store.state.indexSearch.startDay;
                        this.endDay = this.$store.state.indexSearch.endDay;
                        this.stayTime = this.$store.state.indexSearch.stayDay;
                        this.stayMoney = (this.homestay.hprice * this.stayTime).toFixed(1);
                    } else {
                        Notify('获取信息失败');
                    }
                }).catch(error => {
            ...
            detailIsMore() {
                if (this.detailismore === false) {
                    ...
                    this.detailismore = false
                }
            },
            onConfirm(date){
                const [start, end] = date;
                this.show = false;
                this.stayTime = (end.getTime()-start.getTime())/1000/60/60/24;
                this.stayMoney = (this.stayTime*this.homestay.hprice).toFixed(1);
                this.start = start.getMonth()+1+'.'+start.getDate();
                this.end = end.getMonth()+1+'.'+end.getDate();
                let startDay = start.getDay();
                let endDay = end.getDay();
                var weeks = new Array("SUN", "MON", "TUES", "WED", "THU", "FRI", "SAT");
                this.startDay = weeks[startDay];
                this.endDay = weeks[endDay];
            },
            pageback(){
                this.$router.back();
            }
